...
// --- FUNCIÓN GENÉRICA PARA LLAMADAS API (POST) ---
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (response.status === 401) {
                console.error(`[API] Error 401 en ${endpoint}. Sesión expirada. Redirigiendo...`);
                doLogout();
                // Detenemos la ejecución para evitar más errores
        ...
        return await response.text(); // O response.json() si el ESP32 siempre responde JSON
    } catch (error) {
        console.error(`[API] Fallo en la petición a ${endpoint}:`, error);
        throw error; // Relanzamos el error para que el llamador pueda manejarlo si es necesario
    }
}
...
function setFanMode(mode) {
    ...
    apiPost('/fan/mode', { mode }).catch(err => console.error(`[FAN] Error al enviar modo: ${err.message}`));
...
function toggleFanPower() {
    ...
    apiPost('/fan/manual', { power: fanManualPowerState, speed: fanManualSpeed })
        .then(data => console.log(`[FAN] Respuesta del ESP32: ${data}`))
        .catch(err => console.error(`[FAN] Error al enviar estado manual: ${err.message}`));
}
...
function updateFanSlider(value) {
    ...
    if (fanManualPowerState) {
        ...
        // Enviar el comando al ESP32 con la nueva función genérica
        apiPost('/fan/manual', { power: true, speed: fanManualSpeed })
            .then(data => console.log(`[FAN] Respuesta del ESP32: ${data}`))
            .catch(err => console.error(`[FAN] Error al ajustar velocidad: ${err.message}`));
    }
}
...
function registerFanSchedule() {
    ...
    // Enviar el registro al ESP32
    apiPost('/fan/schedule', record)
        .then(data => console.log(`[FAN SCHEDULE] Respuesta del ESP32: ${data}`))
        .catch(err => console.error(`[FAN SCHEDULE] Error al registrar horario: ${err.message}`));
}
...
function assignSchedule() {
    ...
    // Implementar envío de instrucciones al ESP32
    const payload = {
        day: selectedScheduleRecord.day,
        time: selectedScheduleRecord.time,
        speed: selectedScheduleRecord.speed
    };
    apiPost('/fan/schedule/assign', payload)
        .then(data => console.log('[FAN SCHEDULE] Respuesta del servidor:', data))
        .catch(err => console.error(`[FAN SCHEDULE] Error al asignar: ${err.message}`));
...
function controlFanByTemperature(temperature) {
    ...
    if (speedChanged) {
        ...
        apiPost('/fan/temperature', { speed: fanCurrentSpeed })
            .catch(err => console.error(`[FAN TEMP] Error al enviar velocidad: ${err.message}`));
    } else {
